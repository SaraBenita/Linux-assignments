...
#to check if it is a valid airctaft
...
#Delete 3 chracters: " [ ] from all the file
sed -i 's/"//g' arrival_json
sed -i 's/\[//g' arrival_json
sed -i 's/\]//g' arrival_json
sed -i 's/"//g' departure_json
...
#create headerts part in file from the current json
...
if [[ "$isAircraft" == "true" ]]; then
	...
	echo "Data for aircraft $inputApi fetched succesfully!"
else
	echo "Data for airport $inputApi fetched succesfully!"
fi
